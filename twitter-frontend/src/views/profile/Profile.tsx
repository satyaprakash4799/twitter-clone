...
import {deleteTweet, fetchTweets, fetchUser} from "../../store/slices/userSlice";
...
const Profile = () => {
  ...
  const {user: currentUser, loading: currentUserLoading, error: currentUserError} = useSelector((state:RootState)=> state.currentUser)
        ...
        >
          ...
          <Box sx={{padding: 2}}>
            <List>
              ...
              {tweets.tweets.map((tweet, index) => {
                return <TweetItem key={tweet.id} tweet={tweet} currentUser={currentUser} dispatch={dispatch}></TweetItem>;
              })}
            </List>
          </Box>
        </Box>
