...
interface UserState {
  ...
  loadingCreateTweet: boolean;
  createTweetError: string | null;
}

const initFollowState: IFollow = {
  currentPage: 0, limit: 0, totalItems: 0, totalPages: 0, users: [],
};

const initTweetsState: ITweetPagination = {
  currentPage: 0, limit: 0, totalItems: 0, totalPages: 0, tweets: [],
};
...
const initialState: UserState = {
  ...
  createTweetError: null,
  loadingCreateTweet: false,
};

export const fetchCurrentUser = createAsyncThunk<IUser>("currentUser/fetchUser", async () => {
  const {data} = await apiClient.get(`/user`);
  return data?.user as IUser;
});

export const fetchUser = createAsyncThunk<IUser, string>("user/fetchUser", async (userId: string) => {
  const {data} = await apiClient.get(`/user/${userId}`);
  return data?.user as IUser;
});

export const fetchFollowers = createAsyncThunk<IFollow, {
  userId: string; iPage: IPage;
}>("user/followers", async ({userId, iPage}: { userId: string; iPage: IPage }) => {
  const {data} = await apiClient.get(`follow/${userId}/followers?page=${iPage.page}&limit=${iPage.limit}`);
  return data;
});

export const fetchFollowings = createAsyncThunk<IFollow, {
  userId: string; iPage: IPage;
}>("user/followings", async ({userId, iPage}: { userId: string; iPage: IPage }) => {
  const {data} = await apiClient.get(`follow/${userId}/followings?page=${iPage.page}&limit=${iPage.limit}`);
  return data;
});

export const fetchTweets = createAsyncThunk<ITweetPagination, {
  userId: string; iPage: IPage;
}>('user/fetchTweets', async ({userId, iPage}: { userId: string, iPage: IPage }) => {
  const {data} = await apiClient.get(`tweet/all/${userId}?page=${iPage.page}&limit=${iPage.limit}`);
  return data;
})

export const createTweet = createAsyncThunk('user/createTweet', async (tweetData: ITweet) => {
  const {data} = await apiClient.post('tweet', tweetData);
  return data?.tweet;
})
export const deleteTweet = createAsyncThunk('user/deleteTweet', async (tweetId: string) => {
  await apiClient.delete(`tweet/${tweetId}`);
  return tweetId;
})

const currentUserSlicer = createSlice({
  name: "currentUser", initialState: initialStateCurrentUser, reducers: {}, extraReducers: (builder) => {
    builder
...
const userSlicer = createSlice({
  name: "user", initialState: initialState, reducers: {
    ...
    builder
      ...
      .addCase(fetchFollowers.fulfilled, (state, action: PayloadAction<IFollow>) => {
        // state.followers = action.payload;
        const {users}: IFollow = action.payload;
        state.followersLoading = false;
        state.followers = {
          ...action.payload, users: [...state.followers.users, ...users],
        };
      })
      ...
      .addCase(fetchFollowings.fulfilled, (state, action: PayloadAction<IFollow>) => {
        const {users} = action.payload;
        // state.followings = action.payload;
        state.followingsLoading = false;
        state.followings = {
          ...action.payload, users: [...state.followings.users, ...users],
        };
      })
      ...
      .addCase(fetchTweets.fulfilled, (state, action: PayloadAction<ITweetPagination>) => {
        ...
        state.tweets = {
          ...action.payload, tweets: [...state.tweets.tweets, ...newTweets]
        };
      })
      .addCase(createTweet.pending, (state, action) => {
        state.loadingCreateTweet = true;
        state.createTweetError = null;
      })
      .addCase(createTweet.fulfilled, (state, action: PayloadAction<ITweet>) => {
        state.loadingCreateTweet = false;
        const newTweet = [{
          ...action.payload
        }]
        state.tweets.tweets = [ ...newTweet, ...state.tweets.tweets]
      })
      .addCase(createTweet.rejected, (state, action) => {
        state.loadingCreateTweet = false;
        state.createTweetError = action.error.message || 'Something went wrong';
      })
      ...
      .addCase(deleteTweet.fulfilled, (state, action) => {
        ...
        state.tweets.tweets = state.tweets.tweets.filter(tweet => tweet.id !== tweetId);
      });
  },
});
